...
public abstract class ControleCadastroGenerico<T> implements IControleCadastro {
    ...
    protected DaoDocente daoDocente = new DaoDocente();
    protected DaoFuncionario daoFuncionario = new DaoFuncionario();
    ...
    public String[] gerarColunasTabela() {  
        if (classeModelo.equals(Aluno.class)) {
            ...
            return new String[]{"Disicplina", "Semestres"};    
        }else{
    ...
    public String[] getDadosEntidadeModeloParaTabela(T entidade) {
        ...
        if (entidade instanceof Aluno) {
            ...
            return new String[]{disc.getNome(), "" + disc.getSemestre()};       
        }else{
    ...
    public boolean removerCadastro(int index) {
        try {
            Object ERemover = registros.get(index);

            if(ERemover instanceof Aluno){
                return daoAluno.remover((Aluno) ERemover);
            }else if (ERemover instanceof Docente){
                return daoDocente.remover((Docente) ERemover);
            }else if (ERemover instanceof Funcionario){
                return daoFuncionario.remover((Funcionario) ERemover);
            }else if (ERemover instanceof Curso){
                return daoCurso.remover((Curso) ERemover);
            }else if (ERemover instanceof Disciplina){
                return daoDisciplina.remover((Disciplina) ERemover);
            }else{
                System.out.println("Entidade nao encontrada! Erro!");
            }
